...
export function enforceAdminAuth(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        req.role = user.role
        if (req.role !== "admin") {
            return next(new AppError(StatusCodes.FORBIDDEN, 'You do not have admin privileges.'))
        }
        else { next() }
    } catch (e) {
        next(new AppError(StatusCodes.UNAUTHORIZED, 'invalid JWT'))
